...
interface PaymentModalProps {
  ...
  onPaymentSubmitted: (transactionRef: string, viaWhatsapp?: boolean) => void;
}
    ...
    try {
      if (isFirebaseEnabled && db && user?.uid) {
        ...
        if (orderData && !orderId) {
          ...
          const finalTransactionRef = transactionRef.trim() || autoGeneratedRef;
          await updateDoc(doc(db, 'orders', orderId), {
            transactionRef: finalTransactionRef.toUpperCase(),
            paymentMethod: selectedProvider,
            paymentStatus: 'PENDING_VERIFICATION',
            updatedAt: new Date().toISOString()
          });
        ...
        await addDoc(collection(db, 'transactions'), {
          ...
          referenceId: finalTransactionRef.toUpperCase(),
