...
import { PaymentProvider, PaymentConfig, User, Order, Customer } from '../types';
...
export const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const [paymentMethodType, setPaymentMethodType] = useState<'STK_PUSH' | 'MANUAL_MOBILE' | 'MANUAL_BANK' | null>(null);
  ...
  const generateReferenceNumber = (): string => {
    ...
    const prefix = selectedProvider?.substring(0, 3).toUpperCase() || 'PAY';
  ...
  // Determine if provider is mobile money
  const isMobileMoney = (provider: PaymentProvider): boolean => {
    return provider !== 'BANK_TRANSFER';
  };

  const handleProviderSelect = async (provider: PaymentProvider) => {
    setSelectedProvider(provider);
    setShowManualEntry(true); // Enable both STK Push and manual entry
    setStkPushSent(false);
    setPaymentMethodType(null);
    setTransactionRef('');
    setAutoGeneratedRef('');
    
    // Generate auto reference number for STK Push
    const generatedRef = generateReferenceNumber();
    setAutoGeneratedRef(generatedRef);
    
    // Auto-initiate STK Push for mobile money providers
    if (isMobileMoney(provider)) {
      ...
      try {
        ...
        setStkPushSent(true);
        setPaymentMethodType('STK_PUSH');
        showNotification(`STK Push sent to your ${provider.replace('_', ' ')} number`, 'success');
      } catch (error) {
        showNotification('Failed to send STK Push. You can still enter reference manually.', 'error');
        setPaymentMethodType('MANUAL_MOBILE');
      } finally {
        setIsProcessing(false);
      }
    } else {
      // Bank transfer - manual entry only
      setPaymentMethodType('MANUAL_BANK');
    }
  };

  const handleManualEntry = () => {
    setShowManualEntry(true);
    setPaymentMethodType(isMobileMoney(selectedProvider!) ? 'MANUAL_MOBILE' : 'MANUAL_BANK');
  };
  
  const handleUseStkPush = async () => {
    if (!selectedProvider || !isMobileMoney(selectedProvider)) return;
    ...
    try {
      ...
      setStkPushSent(true);
      setPaymentMethodType('STK_PUSH');
      showNotification(`STK Push sent to your ${selectedProvider.replace('_', ' ')} number`, 'success');
    } catch (error) {
  ...
  const handleSubmitPayment = async () => {
    ...
    // Use auto-generated ref if STK Push was used and no manual entry
    ...
    if (finalTransactionRef.length < 6) {
    ...
    try {
      if (isFirebaseEnabled && db && user?.uid) {
        ...
        if (orderData && !orderId) {
          ...
          // Include viaWhatsapp flag if present
          if (orderData.viaWhatsapp !== undefined) {
            orderToCreate.viaWhatsapp = orderData.viaWhatsapp;
          }
        ...
        await addDoc(collection(db, 'transactions'), {
          ...
          paymentMethodType: paymentMethodType || (isMobileMoney(selectedProvider) ? 'MANUAL_MOBILE' : 'MANUAL_BANK'),
        ...
        const viaWhatsapp = orderData?.viaWhatsapp || false;
        onPaymentSubmitted(finalTransactionRef.toUpperCase(), viaWhatsapp);
        showNotification('Order placed! Payment submitted for vendor verification.', 'success');
        
        // Redirect to WhatsApp for WhatsApp orders
        if (viaWhatsapp && sellerId && vendor?.phone) {
          const message = `Hello! I've just placed an order (Ref: ${finalTransactionRef.toUpperCase()}) for TZS ${amount.toLocaleString()}. Please confirm.`;
    ...
    <div className="fixed inset-0 z-[2147483000] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-neutral-900 rounded-xl shadow-2xl w-full max-w-md modal-content">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Payment Method Selection - STK Push or Manual Entry */}
          {selectedProvider && isMobileMoney(selectedProvider) && !stkPushSent && (
            <div className="space-y-3">
              <h3 className="font-semibold text-neutral-900 dark:text-white text-sm">Choose Payment Method</h3>
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={handleUseStkPush}
                  disabled={isProcessing}
                  className="p-4 border-2 border-orange-200 dark:border-orange-800 rounded-lg hover:border-orange-500 bg-orange-50 dark:bg-orange-900/20 transition-all flex flex-col items-center gap-2"
                >
                  <Smartphone className="w-5 h-5 text-orange-600" />
                  <span className="text-sm font-medium text-neutral-900 dark:text-white">STK Push</span>
                </button>
                <button
                  onClick={handleManualEntry}
                  className="p-4 border-2 border-neutral-200 dark:border-neutral-700 rounded-lg hover:border-orange-500 bg-white dark:bg-neutral-800 transition-all flex flex-col items-center gap-2"
                >
                  <CreditCard className="w-5 h-5 text-neutral-600 dark:text-neutral-400" />
                  <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Manual Entry</span>
                </button>
              </div>
            </div>
          )}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-300 mb-2">
                    ...
                    Once completed, your transaction reference code will appear below automatically.
                  </p>
                  {autoGeneratedRef && (
                    <div className="mt-3 p-2 bg-white dark:bg-neutral-900 rounded border border-blue-200 dark:border-blue-700">
                      <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-1">Auto-Generated Reference:</p>
                      <div className="flex items-center gap-2">
                        <p className="font-mono font-bold text-sm text-blue-900 dark:text-blue-200">{autoGeneratedRef}</p>
                        <button
                          onClick={() => handleCopyToClipboard(autoGeneratedRef, 'auto-ref')}
                          className="p-1 hover:bg-blue-100 dark:hover:bg-blue-900 rounded"
                          title="Copy reference"
                        >
                          {copiedField === 'auto-ref' ? (
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <input
                    type="text"
                    value={transactionRef || autoGeneratedRef}
                    onChange={(e) => setTransactionRef(e.target.value.toUpperCase())}
                    placeholder={autoGeneratedRef || "e.g., QWF82937JK"}
                    className="w-full p-3 pr-12 border border-neutral-200 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800 text-neutral-900 dark:text-white focus:ring-2 focus:ring-orange-500 outline-none font-mono text-sm"
                    maxLength={20}
                  />
                  {(transactionRef || autoGeneratedRef) && (
                    <button
                      onClick={() => handleCopyToClipboard(transactionRef || autoGeneratedRef, 'ref-code')}
                      className="absolute right-2 top-1/2 -translate-y-1/2 p-2 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded transition-colors"
                      title="Copy reference code"
                    >
                      {copiedField === 'ref-code' ? (
                ...
                <p className="text-xs text-neutral-500 mt-1">
                  {autoGeneratedRef && !transactionRef 
                    ? "Auto-generated reference code. You can edit it if needed."
                    : "Enter the reference code from your payment confirmation message"}
                </p>
              </div>

              {isMobileMoney(selectedProvider!) && stkPushSent && (
                <button
                  onClick={handleManualEntry}
                  className="w-full p-3 border border-neutral-200 dark:border-neutral-700 rounded-lg text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-800 text-sm"
                >
                  Enter reference code manually instead
                </button>
              )}
            </div>
          )}
        ...
        <div className="p-6 border-t border-neutral-200 dark:border-neutral-800 flex gap-3">
          ...
          <button
            ...
            disabled={isProcessing || !selectedProvider || !(transactionRef.trim() || autoGeneratedRef) || (transactionRef.trim() || autoGeneratedRef).length < 6}
